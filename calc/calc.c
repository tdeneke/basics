...
struct token_queue infix_to_postfix(struct token_queue * pqueue_infix) {
    ...
    while(ptoken = dequeue(pqueue_infix)){
        if(ptoken->type == OPERAND){
            ...
            while(ptop && (op_precedences[ptop->value.op_code]>op_precedences[ptoken->value.op_code] || (op_precedences[ptop->value.op_code] == op_precedences[ptoken->value.op_code] && op_associativity[op_precedences[ptop->value.op_code]]==LEFT) )){
                p_expr_token ttoken = pop(&ptop);
                enqueue(&queue_postfix, ttoken);
            }
            push(&ptop, ptoken);
        }
    }
...
double evaluate_postfix(struct token_queue * pqueue_postfix) {
    ...
    p_expr_token ptoken = NULL;
    p_expr_token result_token = NULL;
    union token_value value;
    p_expr_token ptop = NULL;
    ...
    while(ptoken = dequeue(pqueue_postfix)){
        ...
        if(ptoken->type == OPERATOR){
            if(ptoken->value.op_code == NEGATE){
                value.operand = -1 * pop(&ptop)->value.operand;
                push(&ptop, new_token(OPERAND, value));
            }else{
                switch(ptoken->value.op_code){
                    case ADD:
                        value.operand = pop(&ptop)->value.operand + pop(&ptop)->value.operand;
                        break;
                    case SUBTRACT:
                        value.operand = pop(&ptop)->value.operand - pop(&ptop)->value.operand;
                        break;
    ...
    // result_token = pop(&ptop);
    // return result_token->value.operand;
